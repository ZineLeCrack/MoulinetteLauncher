...
cp "$script_dir/ft_stock_str.h" "$src_dir"
/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_strs_to_tab.c" "$script_dir/main.c" -o "$executable"
...
if [[ $? -ne 0 ]]; then
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			norminette -R CheckForbiddenSourceHeader "$src_dir/ft_strs_to_tab.c" > "$script_dir/user_output"

			if [[ $? -ne 0 ]]; then

				echo -e "$RED"
				norminette -R CheckForbiddenSourceHeader "$src_dir/ft_strs_to_tab.c" | grep "Error"
				echo -e "$RESET"
				echo -e "$RED>>>>>>>>>>>>>>>>>>>>>>>>>>>>> FAILURE <<<<<<<<<<<<<<<<<<<<<<<<<<<$RESET"
	...
	rm -f "$executable" "$script_dir/user_output" "$src_dir/ft_stock_str.h"
fi
