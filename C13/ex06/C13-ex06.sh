...
/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/btree_level_count.c" "$script_dir/main.c" -o "$executable"
...
if [[ $? -ne 0 ]]; then
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			norminette -R CheckForbiddenSourceHeader "$src_dir/btree_level_count.c" | grep "Error"
