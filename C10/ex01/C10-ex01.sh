...
src_dir="ex01"
expected_executable="$script_dir/expected_ft_cat"
user_executable="$script_dir/user_ft_cat"
executable="$script_dir/ft_cat"
...
function test_stdin() {
	...
	echo -n "stderr : " > "user_stderr_output"
	ln -s "$expected_executable" "$executable"
	cat $@ | "$executable" >> "expected_stdin_output" 2>> "expected_stderr_output"
	echo "Exit status:" $? >> "expected_output"
	rm -f "$executable"
	ln -s "$user_executable" "$executable"
	cat $@ | "$executable" >> "user_stdin_output" 2>> "user_stderr_output"
	echo "Exit status:" $? >> "user_output"
	rm -f "$executable"
	cat "expected_stdin_output" "expected_stderr_output" >> "expected_output"
	cat "user_stdin_output" "user_stderr_output" >> "user_output"
...
if [[ $? -ne 0 ]]; then
	...
	mv "ft_cat" "$script_dir/user_ft_cat"
	rm -f "$executable"
	...
	test "error.txt" "test.txt" "test.txt"
	...
	test "test.txt" "no_perm.txt"

	echo >> "expected_output"
	echo >> "user_output"

	test "no_perm.txt" "test.txt" "error.txt"

	echo >> "expected_output"
	echo >> "user_output"

	test "test.txt" "test.txt" "test.txt" "test.txt" "test.txt"
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			norminette -R CheckForbiddenSourceHeader > "user_output"
	...
	rm -f "expected_output" "expected_stderr_output" "expected_stdin_output" "user_output" "user_stdin_output" "user_stderr_output" "$script_dir/user_ft_cat"
	cd "$OLDPWD"
