...
/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_list_clear.c" "$script_dir/main.c" -o "$executable"
...
if [[ $? -ne 0 ]]; then
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			norminette -R CheckForbiddenSourceHeader "$src_dir/ft_list_clear.c" | grep "Error"
