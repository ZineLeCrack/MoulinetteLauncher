...
/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_sorted_list_insert.c" "$script_dir/main.c" -o "$executable"
...
if [[ $? -ne 0 ]]; then
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			norminette -R CheckForbiddenSourceHeader "$src_dir/ft_sorted_list_insert.c" | grep "Error"
