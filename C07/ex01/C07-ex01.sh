...
/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_range.c" "$script_dir/main.c" -o "$executable"
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			if [[ $? -ne 0 ]]; then
				...
				/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_range.c" "$script_dir/failmalloc.c" -o "$executable"
