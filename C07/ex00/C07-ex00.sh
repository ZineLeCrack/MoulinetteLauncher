...
/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_strdup.c" "$script_dir/main.c" -o "$executable"
	...
	if [[ $? -ne 0 ]]; then
		...
		if [[ $? -ne 0 ]]; then
			...
			if [[ $? -ne 0 ]]; then
				...
				/bin/cc -Wall -Wextra -Werror -g3 "$src_dir/ft_strdup.c" "$script_dir/failmalloc.c" -o "$executable"
